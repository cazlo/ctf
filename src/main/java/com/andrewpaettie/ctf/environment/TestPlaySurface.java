package com.andrewpaettie.ctf.environment;

import com.andrewpaettie.ctf.agent.*;
...
public class TestPlaySurface extends Frame implements StateChangeListener {
	...
	public TestPlaySurface() {
		...
		File setFile = new File(classLoader.getResource( mSetsDirName ).getFile());
		...
		mDelayChoice.select( "100" );
		mObstacleChoice.select( "Large Random" );
		mTeam1Choice.select( "ctf.agent.Agent007" );
		mTeam2Choice.select( "ctf.agent.Agent014" );
		...
		try {
			StreamTokenizer tokenizer = 
				new StreamTokenizer(classLoader.getResourceAsStream( agentClassesFileName ));
		...
		mTeamClasses[0] = mAvailableClasses[3];
		mTeamClasses[1] = mAvailableClasses[4];
	...
	private class GameThread extends Thread {
		public void run() {
			String delayName = "50";// mDelayChoice.getSelectedItem();
			try {
	...
	private class ObstacleChoiceListener implements ItemListener {
		public void itemStateChanged( ItemEvent inIE ) {		
			...
			try {
				FileSet fileSet = new FileSet( fileName );
				mBoardSet = fileSet;
